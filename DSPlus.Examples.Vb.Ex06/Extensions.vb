...
Module Extensions
    ...
    Public Sub SetProperty(Of TCtl As Control, TProp)(ByVal control As TCtl, ByVal propexpr As Expression(Of Func(Of TCtl, TProp)), ByVal value As TProp)
        ...
        Dim disp = control.Dispatcher

        ' check if dispatching is required
        If Not disp.CheckAccess() Then
            ' if it is, perform it
            ' this will invoke this method from (hopefully) the control's thread
            disp.Invoke(New SetPropertyDelegate(Of TCtl, TProp)(AddressOf SetProperty), control, propexpr, value)
            Return
        End If
    ...
    Public Function GetProperty(Of TCtl As Control, TProp)(ByVal control As TCtl, ByVal propexpr As Expression(Of Func(Of TCtl, TProp))) As TProp
        ...
        If Not disp.CheckAccess() Then
            ' if it is, perform it
            ' this will invoke this method from (hopefully) the control's thread
            Return DirectCast(disp.Invoke(New GetPropertyDelegate(Of TCtl, TProp)(AddressOf GetProperty), control, propexpr), TProp)
        End If
    ...
    Public Sub InvokeAction(Of TCtl As Control)(ByVal control As TCtl, ByVal dlg As [Delegate], ByVal ParamArray args As Object())
        ...
        If Not disp.CheckAccess() Then
            ' if it is, perform it
            ' this will invoke this method from (hopefully) the control's thread
            disp.Invoke(New InvokeActionDelegate(Of TCtl)(AddressOf InvokeAction), control, dlg, args)
            Return
        End If
    ...
    Public Function InvokeFunc(Of TCtl As Control, TResult)(ByVal control As TCtl, ByVal dlg As [Delegate], ByVal ParamArray args As Object()) As TResult
        ...
        If Not disp.CheckAccess() Then
            ' if it is, perform it
            ' this will invoke this method from (hopefully) the control's thread
            Return DirectCast(disp.Invoke(New InvokeFuncDelegate(Of TCtl, TResult)(AddressOf InvokeFunc), control, dlg, args), TResult)
        End If
